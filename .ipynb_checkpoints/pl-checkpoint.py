...
def norm_FR_ES(paths=paths, region='Hpc', celltype='Pyr', min=0, max=2040, bin=1, min_dur=30, pre_post=False):
    ...
    -> array with population firing rate z-score for specified celltype and brainregion for each ES
    """
...
def interval_pop_FR(path, region='Hpc', celltype='Pyr', min=0, max=2040, bin=1, whole=True):
    """
    inputs:
    min, max, bin is in seconds
    if whole is True, the interval will be from 0 to the end of recording. It overwrites the min and max specified.
    ...
    hist=np.array(hist)
    cell_type=hist[(metadata.Type==celltype) &(metadata.Region==region)]
    pop_fr=np.mean(cell_type, axis=0)
    
    return pop_fr, e

def pop_FR_ES(paths=paths, region='Hpc', celltype='Pyr', bin=1, min_dur=30, pre_post=False):
    """
    bin is in seconds
    min_dur is in mins
    pre_post: select True if you want the pre and post RUN Es to be separated
    -> array with population firing rate for specified celltype and brainregion for each ES aranged in bins of specified time length
    """
    ...
    for i in range(len(useful_paths_pre)): # going over all sessions
        ...
        for n in range(len(ES_pre)):
            start=ES_pre[n][0]['start'].iloc[0]/1e6
            stop=ES_pre[n][0]['stop'].iloc[-1]/1e6
            c, e=interval_pop_FR(useful_paths_pre[i],region, celltype, min=start, max=stop,bin=bin, whole=False)
            all_ES_pre.append(c)
    ...
    for i in range(len(useful_paths_post)):
        ...
        for n in range(len(ES_post)):
            start=ES_post[n][0]['start'].iloc[0]/1e6
            stop=ES_post[n][0]['stop'].iloc[-1]/1e6
            c, e=interval_pop_FR(useful_paths_post[i],region, celltype, min=start, max=stop, bin=bin, whole=False)
...
def pop_FR_ES_state_bin(paths=paths, region='Hpc', celltype='Pyr', state='nrem', bin=1, min_dur=30, pre_post=False):
    """
    bin is in seconds
    min_dur is in mins
    pre_post: select True if you want the pre and post RUN Es to be separated
    Unlike pop_FR_ES_state, it gives avg FR rate per minute (i.e. multiple values for each REM epoch instead of just one.)
    -> array with average population firing rate/time bin for specified celltype and brainregion for each epoch of the specified state in all ESs. Mean FR is computed in bins of specified time length
    """
